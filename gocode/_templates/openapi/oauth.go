...
func VerifyOAuth(
	authorization string, scopes []string, certPath string,
) error {
	verifyKey, err := ParsePublicKeyFromPEM("cert/nrf.pem")
	if err != nil {
	...
	if !verifyScope(token.Claims.(*models.AccessTokenClaims).Scope, scopes) {
		return errors.Wrapf(err, "verify OAuth scope")
	}
	return nil
}

func verifyScope(scope string, scopes []string) bool {
	if len(scope) != 0 {
...
var cachedTokens map[string]*oauth2.Token

func GetCachedAT(scope string) *oauth2.Token {
	if cachedTokens == nil {
		cachedTokens = make(map[string]*oauth2.Token)
	}

	if cachedTokens[scope] != nil {
		// Make sure token is valid for at least the next 3 seconds
		now := time.Now()
		if now.Add(time.Second * 3).After(cachedTokens[scope].Expiry) {
			return nil
		} else {
...
func PutCachedAT(scope string, t *oauth2.Token) {
	if cachedTokens[scope] != nil {
		// See if t is valid longer than currently cached token
		if t.Expiry.After(cachedTokens[scope].Expiry) {
			// Replace it
			cachedTokens[scope] = t
		}
	} else {
		cachedTokens[scope] = t
	}
