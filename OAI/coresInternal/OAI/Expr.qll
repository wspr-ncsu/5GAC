...
class HttpMethodArgument extends BL::Expr {
    CoreServiceCall serviceCall;
    HttpMethodArgument(){
        exists(CoreServiceCall s | s.getTargetCore().getName() = "curl_http_client" 
            and
            this = s.getArgument(1) |
            serviceCall = s)
        or
        exists(CoreServiceCall s | s.getTargetCore().getName() = "curl_create_handle" 
            and
            s.getTargetCore().getNumberOfParameters() = 5
            and
            this = s.getArgument(3)|
            serviceCall = s)
        or
        exists(CoreServiceCall s | s.getTargetCore().getName() = "curl_create_handle" 
            and
            s.getTargetCore().getNumberOfParameters() = 6
            and
            this = s.getArgument(4)|
            serviceCall = s)
        or
        exists(CoreServiceCall s | s.getTargetCore().getName() = "curl_create_handle" 
            and
            s.getTargetCore().getNumberOfParameters() = 7
            and
            this = s.getArgument(5)|
            serviceCall = s)
        or
        exists(CoreServiceCall s | s.getTargetCore().getName() = "curl_create_handle" 
            and
            s.getTargetCore().getNumberOfParameters() = 8
            and
            this = s.getArgument(5)|
            serviceCall = s)
        or
        exists(CoreServiceCall s | s.getTargetCore().getName() = "send_request" 
            and
            s.getTargetCore().getNumberOfParameters() = 6
            and
            this = s.getArgument(2)|
            serviceCall = s)
        }

        CoreServiceCall getServiceCall(){
            result = serviceCall
        }

        BL::Variable getVariableTarget(){
            result = this.(BL::FunctionCall).getArgument(_).(BL::VariableAccess).getTarget()
            or
            result = this.(BL::VariableAccess).getTarget()
        }

        BL::FunctionCall getAssignment(){
            (result = getVariableTarget().getAnAccess().getParent().(BL::FunctionCall)
            and
            result.getTarget().getName() = "operator=")
        }

        BL::FunctionCall getClosestAssignment(){
            // exists(BL::ControlFlowNode c, BL::FunctionCall f | c = getAssignment() and f = getAssignment()
            //         and c = serviceCall.getAPredecessor+()
            //         and not
            //         (c.getASuccessor+() = f and serviceCall.getAPredecessor+() = f and f = c)
            //         | result = c)

            exists(BL::FunctionCall assign | assign = getAssignment() and assign = serviceCall.getAPredecessor+() | result = assign)
            and
            not exists(BL::FunctionCall assign, BL::FunctionCall f | assign = result and f = getAssignment()
                and assign.getASuccessor+() = f and serviceCall.getAPredecessor+() = f)
        }

        string getAssignedValue(){
            result = this.(BL::FunctionCall).getArgument(_).(BL::StringLiteral).toString()
            or
            (this.toString() = "json_part" and result = "GET")
            or
            if exists(getClosestAssignment())
                then result = getClosestAssignment().getArgument(_).(BL::StringLiteral).toString()
            else result = getVariableTarget().getInitializer().getExpr().(BL::FunctionCall).getArgument(_).(BL::StringLiteral).toString()
...
class UriArgument extends BL::Expr {
    CoreServiceCall serviceCall;
    CoreFunction enclosingFunc;

    UriArgument(){
        exists(CoreServiceCall s | this = s.getArgument(0) | serviceCall = s and enclosingFunc = this.getEnclosingFunction())
        // and not
        // (this.(BL::VariableAccess).getTarget() instanceof BL::Parameter
        // or
        // this.toString() = "r_endpoint")
    }

    CoreServiceCall getServiceCall(){result = serviceCall}

    CoreFunction getFunctions(){result = enclosingFunc}

    // string getAssignedValue(){
    //     result = getInitializedString() + getAddedStrings()
    //     or
    //     result = getAssignedStrings()
    // }

    string getUriString(){
        exists(getInitializedString()) and exists(getConcatString()) and result = getInitializedString() + "/" + getConcatString()
        or
        exists(getAssignString()) and exists(getConcatString()) and result = getAssignString() + "/" + getConcatString()
        or not exists(getConcatString())
        and
            (exists(getInitializedString()) and result = getInitializedString()
            or
            exists(getAssignString()) and result = getAssignString())
        or
        not (exists(getAssignString()) or exists(getInitializedString())) and result = getConcatString()
        or not (exists(getAssignString()) or exists(getInitializedString()) or exists(getConcatString())) and result = "$FUNC "+ toString()
    }

    string getInitializedString(){
        if getInitializer() instanceof OpPlusCall
        then result = buildStringFromFunctionCall(getInitializer())
        else if getInitializer().(BL::FunctionCall).getTarget().getName() = "basic_string" 
            then result = buildStringFromFunctionCall(getInitializer())
            and getInitializer().(BL::FunctionCall).getNumberOfArguments() > 0
        else result = "$FUNC " + getInitializer().(BL::FunctionCall).getTarget().getName()
    }

    string getAssignString(){
        if getClosestAssign() instanceof OpPlusCall
        then result = buildStringFromFunctionCall(getClosestAssign())
        else if getClosestAssign().(BL::FunctionCall).getTarget().getName() = "basic_string" 
            then result = buildStringFromFunctionCall(getClosestAssign())
            and getClosestAssign().(BL::FunctionCall).getNumberOfArguments() > 0
        else result = "$FUNC " + getClosestAssign().(BL::FunctionCall).getTarget().getName()
    }

    BL::FunctionCall getInitializer(){
        (result = this.(BL::VariableAccess).getTarget().getInitializer().getExpr()
        or
        result = this.(BL::FunctionCall).getArgument(_).(BL::VariableAccess).getTarget().getInitializer().getExpr()
        )
        // and
        // result instanceof OpPlusCall
            
    }
    
    BL::FunctionCall getInitializerBasicString(){
        result = this.(BL::VariableAccess).getTarget().getInitializer().getExpr()
        and
        result.(BL::FunctionCall).getTarget().getName() = "basic_string"    
    }

    BL::Expr getAssign(){
        result = getAnAccess().getParent().(OpEqCall).getArgument(0)
        or
        result = getAnAccess().getParent().(GetUriCall)

    }

    BL::Expr getClosestAssign(){
        // Shortcut TODO: Handle a getter function
        if getAssign() instanceof GetUriCall
        then result = getAssign()
        else
            result = getAssign() and
            not exists(BL::Expr assign, BL::FunctionCall f | assign = result and f = getAssign()
                and assign.getASuccessor+() = f and this.getAPredecessor+() = f)
            and not result = getASuccessor+()

        // exists(BL::Expr assign, BL::FunctionCall f | assign = getAssign() and f = getAssign()
        //     and not (assign.getASuccessor+() = f and this.getAPredecessor+() = f)
        //     and not assign=getASuccessor+()
        //     | result = assign )
    }

    BL::VariableAccess getAnAccess(){
        result = this.(BL::VariableAccess).getTarget().getAnAccess()
        or
        result = this.(BL::FunctionCall).getArgument(_).(BL::VariableAccess).getTarget().getAnAccess()
    }

    string getConcatString(){
        exists(OpPlusEqCall op | 
            op.(BL::FunctionCall).getQualifier().(BL::VariableAccess).getTarget().getAnAccess() = this
            or
            op.(BL::FunctionCall).getQualifier().(BL::VariableAccess).getTarget().getAnAccess() = this.(BL::FunctionCall).getArgument(0)
        | result = buildStringFromFunctionCall(op))
    }
        // string getInitializerValue(){
